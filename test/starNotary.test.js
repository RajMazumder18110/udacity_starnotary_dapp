...
contract('StarNotary', (accounts) => {
    ...
    describe('Put for sale', () => {
        it('Should put user1 created star on sale', async () => {
            const starPrice = web3.utils.toWei('0.0001', 'ether');
    ...
    describe('Exchange Stars', () => {
        it('Should exchange user2 star with user3', async () => {
            ...
            assert.equal(
                result, user3,
                "Current token2's owwner is not account 3"
            )
            result = await contract.ownerOf(user3Token);
            assert.equal(
                result, user2,
                "Current token3's owwner is not account 2"
            )
        })
    })
    ...
    describe('Token transfer', () => {
        it('should transfer user4 token to user5', async () => {
            const user4Token = 4
            await contract.transferStar( user5, 4, { from: user4 })
